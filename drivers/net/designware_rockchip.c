...
struct rk_gmac_ops {
	...
	const u32 *regs;
	void (*set_clock_selection)(struct eqos *eqos, bool input, bool enable);
};
...
static void rk3562_set_to_rgmii(struct eqos *eqos,
				int tx_delay, int rx_delay)
{
	...
	if (IS_ERR(priv->grf)) {
		dev_err(dev, "Missing rockchip,grf or rockchip,grf property\n");
		return;
	}
	...
	regmap_write(priv->grf, RK3562_GRF_SYS_SOC_CON0,
		     RK3562_GMAC0_CLK_RGMII_MODE);

	regmap_write(priv->grf, RK3562_GRF_IOC_GMAC_IOFUNC0_CON1,
		     DELAY_ENABLE(RK3562, tx_delay, rx_delay));
	regmap_write(priv->grf, RK3562_GRF_IOC_GMAC_IOFUNC0_CON0,
		     DELAY_VALUE(RK3562, tx_delay, rx_delay));

	regmap_write(priv->grf, RK3562_GRF_IOC_GMAC_IOFUNC1_CON1,
		     DELAY_ENABLE(RK3562, tx_delay, rx_delay));
	regmap_write(priv->grf, RK3562_GRF_IOC_GMAC_IOFUNC1_CON0,
		     DELAY_VALUE(RK3562, tx_delay, rx_delay));
}

static void rk3562_set_to_rmii(struct eqos *eqos)
{
	struct eqos_rk_gmac *priv = to_rk_gmac(eqos);
	struct device *dev = priv->dev;
...
static void rk3562_set_gmac_speed(struct eqos *eqos, int speed)
{
	struct eqos_rk_gmac *priv = to_rk_gmac(eqos);
	struct device *dev = priv->dev;
	unsigned long rate;
...
static void rk3562_set_clock_selection(struct eqos *eqos, bool input,
					bool enable)
{
	struct eqos_rk_gmac *priv = to_rk_gmac(eqos);
	struct device *dev = priv->dev;
	...
	if (IS_ERR(priv->grf) || IS_ERR(priv->grf)) {
		dev_err(dev, "Missing rockchip,grf or rockchip,grf property\n");
		return;
	}
	...
	if (!priv->bus_id) {
		...
		regmap_write(priv->grf, RK3562_GRF_SYS_SOC_CON0, value);

		value = input ? RK3562_GMAC0_IO_EXTCLK_SELET_IO :
				RK3562_GMAC0_IO_EXTCLK_SELET_CRU;
		regmap_write(priv->grf, RK3562_GRF_IOC_GMAC_IOFUNC0_CON1, value);
		regmap_write(priv->grf, RK3562_GRF_IOC_GMAC_IOFUNC1_CON1, value);
	} else {
		...
		regmap_write(priv->grf, RK3562_GRF_SYS_SOC_CON1, value);
...
static int rk_gmac_powerup(struct eqos *eqos)
{
	...
	if (priv->ops->set_clock_selection)
		priv->ops->set_clock_selection(eqos, priv->clock_input, true);
