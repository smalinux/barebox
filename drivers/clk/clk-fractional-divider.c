				 ...
				 struct clk_rate_request *req)
{
	...
	if (!req->rate || (!(clk->flags & CLK_SET_RATE_PARENT) && req->rate >= req->best_parent_rate)) {
	...
	if (fd->approximation)
		...
		clk_fractional_divider_general_approximation(hw, req->rate, &req->best_parent_rate, &m, &n);
